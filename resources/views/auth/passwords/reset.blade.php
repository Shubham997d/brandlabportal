@component('layouts.app')
    ...
    <div class="md:bg-white container md:mx-4 md:mx-auto md:shadow-lg rounded md:mt-32 flex md:flex-row flex-col md:max-w-2xl lg:max-w-4xl justify-between auth-page">
        <div class="md:w-1/2 text-center md:flex flex-col items-center px-8 py-4">
            <p class="text-teal-500 font-bold text-4xl">{{ config('app.name', 'BrandLab Portal') }}</p>
        ...
        <div class="w-full md:bg-indigo-100 md:w-1/2 p-8">
            <h3 class="text-gray-500 font-medium">@lang('auth.Reset Password')</h3>
            @if (session('status'))
            ...
            <form role="form" method="POST" action="{{ url('/password/reset') }}">
                ...
                <p class="py-4">
                    <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded auth-btn">@lang('auth.Reset Password')</button>
                </p>
