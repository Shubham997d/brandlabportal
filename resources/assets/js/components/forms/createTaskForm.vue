<template>
<div v-if="formShown">
  <div class="absolute top-0 left-0 right-0 mt-24 container mx-auto px-4 sm:max-w-xl md:max-w-3xl lg:max-w-5xl xl:max-w-6xl z-30 task-container-popup">
      ...
      <div class="bg-white flex flex-row flex-wrap justify-between lg:flex-no-wrap pt-4">
       ...
       <!--  <div>
        ...
        <div>
            ...
            <div class="flex flex-row items-center mx-8 py-2 relative">
                <div @click="toggleStatusMenu" v-click-outside="closeStatusMenu" :style="'background-color: ' + statusColourUpdated"  class="px-4 py-1 mt-1 text-gray-900 font-semibold rounded-full cursor-pointer task-status">
                {{ statusTextUpdated ? statusTextUpdated : statusDefaultText }}
            </div>
        ...
        <div>
            <label class="text-sm text-gray-600 px-8" for="cycle">
                {{ 'Task Duration' | localize }}
            </label>
            <div class="flex flex-row items-center mx-8 py-2 relative">
               <vue-timepicker v-model="taskDuration" ref="taskDuration" format="hh:mm" :minute-interval="10"></vue-timepicker>
            </div>
        </div>
        <!-- Related To -->
<!--         <div>
...
export default {
  ...
  data: () => ({
    ...
    statusDefaultColour: '#f4cbcc',
    statusDefaultText: 'Incomplete',
    statuses: [],
    statusId: null,
    taskDuration: null,
    statusColourUpdated: null,
    statusTextUpdated: null,
  }),
  ...
  computed: {
    ...
    statusBackground () {
      return statusColourUpdated ? statusColourUpdated : statusDefaultColour
    }
  },
  ...
  methods: {
    ...
    /*Set Default Cycle Id To 0 For No Cycle*/
    ...
    changeStatus (id) {      
      this.statusId = id      
      this.statusColourUpdated  = this.getStatusValues(this.statuses,this.statusId,'color')
      this.statusTextUpdated  = this.getStatusValues(this.statuses,this.statusId,'name')
    ...
    getStatusValues (statuses,id,field) {
      var result;   
      for( var i = 0, len = statuses.length; i < len; i++ ) {        
        if( statuses[i]['id'] == id ) {
            result = statuses[i][field];
            break;
        }
      }
      return result;
