  ...
  <div class="bg-white rounded shadow py-8 mt-8">
    <div class="flex flex-col py-4">
      ...
      <div class="mb-4 px-8">
        ...
        <div class="relative">
          <input @keyup="checkUsernameAvailability" maxlength="40" v-model='user.username' class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-800" id="username" type="text" placeholder="Nehal Hasnayeen">
      ...
      <div class="my-6 px-8">
        <a @click="update" class="main-button" href="#">
          {{ 'Update' | localize }}
        </a>
      </div>
...
export default {
  ...
  methods: {
    ...
    update () {
      ...
      axios.put('data/users/' + this.user.username + '/account', params)
        .then((response) => {          
        ...
        .catch((error) => {          
          for (const prop in error.response.data.errors) {
            this.showNotification({type: 'error', message: error.response.data.message})
